...
this file gives Claude Code (claude.ai/code) guidance for working with code in this repository.

---

## Repo layout (pivoted back 2026-07-25)

the project pivoted to Mamba-as-MHA on 2026-07-04. then Kolin sir emailed 2026-07-25 asking to continue the summer work toward two thesis pieces, so the project pivoted back to the kraken2/dorado thesis work. both directions are real; only one is active at a time.
...
**if you are asked to work on Dorado, Kraken2, Luna, Orion/Jetson profiling, perf commands, the optimisation patch, AccuracyDrift, the k-mer cache thesis, cell-width/double-hashing, or Centrifuge comparisons** - read `dorado-kraken-research/CLAUDE.md` first. that file's machine list, key paths, standard profiling commands, and critical facts are all still correct; only the location moved.
...
Kolin sir emailed 2026-07-25 asking to continue the summer kraken2 work toward **two thesis pieces** ("smaller database + smarter cache"). both benchmark against **Centrifuge** (not previously evaluated in this repo - only mentioned in passing as background in `dorado-kraken-research/docs/knowledge_base.md`):
...
**Mamba-as-MHA direction (paused 2026-07-25):** this direction explores whether Mamba (a state-space sequence model) can be reformulated as an MHA-shaped computation for hardware efficiency, based on the Mamba-2 State Space Duality result (Dao & Gu, 2024). the user has zero ML background, so any explanation/doc/comment aimed at them must define terms on first use. target hardware was never decided (Luna vs Orion vs both). resume from `MAMBA_MHA_EXPLAINER.md` §6 when picked back up.
