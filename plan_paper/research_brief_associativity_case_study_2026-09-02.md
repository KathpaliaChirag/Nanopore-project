...
both are real, not mutually exclusive; allocation/first-touch cost likely dominates, based on the DB/thread-dependence pattern already measured: `sample_targeted`/T=1 loses 80% at 64-way (a short run, allocation cost not amortized) vs `pluspf_103gb`/T=32-96 losing ~0% (a long run, allocation cost amortized away). pure per-lookup scan cost should scale with lookup volume, not run length, so it doesn't explain this shape as cleanly as first-touch cost does. not confirmed, needs the profiling plan below.
...
- the original "22x slower / 85% LLC-miss" size-sweep cliff (2026-08-26) is **superseded**, later diagnosis (S3.0, 2026-08-30) showed most of it was a `thread_local` static-array TLS/stack-budget bug, not a generic large-array cost; S3.0 alone dropped the "old" baseline from 12.51s to ~1.18s.
...
2. **Double hashing**, `second_hash()` already fully implemented in Kraken2 v2.17.1, dead behind `-DLINEAR_PROBING`. flipping it is a compile-flag change, but requires a DB rebuild (a real but bounded cost), never measured on Luna. near-zero engineering cost for a paper-relevant number.
...
2. build a standalone synthetic microbenchmark that only allocates + first-touches a `S2_NUM_SETS x S2_WAYS` thread-local array and exits (no kraken2, no hash table), sweep ways 4->64 to get a clean allocation-cost curve, subtract from step 1's deltas to estimate the scan-cost residual.
