...
**S3.3 is done, verified, and committed+tagged on Luna as `safe/S3.3` (`b8c1ee0`).** S3 (sizing) is now feature-complete per this week's scope: S3.0 (crash fix) + S3.1/S3.2 (LLC-topology-aware, thread-scaled formula) + S3.3 (residual-slowdown fix). Still open: S3.2's empirical sweep of `f` (currently 0.25, a placeholder).

### 2026-08-30 — S3.4: full DB × thread-count benchmark finds no measurable wall-clock win, and explains exactly why
**Command:** `plan_paper/scripts/compare_s3_final.py`, per the user's suggestion — S0 (no cache) vs. `kraken2-fresh-bin-s2-baseline` (built fresh from `classify.cc.pre-s3.0.bak`, the exact original committed `safe/S2.4` state: fixed 4,096 sets, `thread_local` static array, non-zero sentinel + `new[]`) vs. `kraken2-fresh-bin-s2` (today's real, fully-wired S3.0+S3.1/S3.2+S3.3 build), across all 3 DBs × 6 thread counts (1/8/16/32/64/96), 3 interleaved runs each, launched inside `tmux` on Luna.

**Result — a real, low-noise null result, not a failure:** all three binaries land within noise of each other at every single DB/thread-count cell (CV% almost entirely under 1%, a few cells up to 4.6% — a trustworthy, low-variance measurement, not signal buried in noise). No configuration shows a measurable speedup from either S2-baseline or S2-final over S0.

**Why, precisely — two separate findings that are easy to conflate but aren't the same thing:**
1. The 2026-08-26 finding (`S2.4 measured` entry, above) already established the 4-way cache shows essentially no hit-rate benefit at the sizes it actually runs at — even the best-tested config (262,144 sets, with pinning) only reached a 1.48% hit rate. Kraken2's minimizer space is enormous relative to a few hundred thousand cache slots; almost every lookup still goes to the real hash table regardless of cache implementation.
2. Today's S3.0/S3.3 fixes address crash and slowdown *bugs* that only manifest at **large** sizes (≥262,144 for the crash, ≥1,048,576 for the slowdown) — sizes the sizing formula, correctly and conservatively (`f=0.25`, clamped to `[4096, 262144]`), never asks for in normal operation across any tested thread count. The formula being "self-limiting" at realistic thread counts was explicitly predicted by `plan_paper/s3_s4_debate_report_2026-08-27.md`'s Q2 ("a properly-scaled formula mostly avoids [the ceilings] by construction") — today's data confirms that prediction directly, and explains why fixing those bugs was never going to show up as a wall-clock win in this specific benchmark: the formula doesn't operate anywhere near where the bugs lived.

**Honest conclusion for the paper:** S3's crash/slowdown fixes are real, necessary correctness work — they unblock safely exploring larger cache sizes and are a prerequisite for S4's eviction work to build on a stable base — but they don't themselves produce a measurable performance win, because the formula was deliberately designed to stay small and safe rather than chase raw capacity. This reinforces, with real full-matrix data, the debate's original reason for prioritizing S4 (eviction policy) over further S3 (sizing) work: eviction *policy* is the lever that already showed a real, measured effect (+25% relative hit-rate gain at the smallest, most realistic capacity, from the 2026-08-26 pinning experiment) — raw capacity/sizing tuning alone does not, at any tested thread count or DB. **S3 is complete; S4 is next, with stronger justification than before.**

### 2026-08-30 — S4.0: occupancy/reuse-distance diagnostic finds a real hashing bug, bigger than anything else found today
**Command:** `plan_paper/scripts/s4_0_diagnostic_patch.py` — a standalone variant (same wiring as `s2_standalone_patch.py`) adding a per-set occupancy counter and a log-scale reuse-distance histogram, T=1 only (non-timed, `unordered_map`-based). Run against `standard_8gb`, single-threaded, same fastq as every prior test.

**Result — a real, previously-undiscovered bug:** `S2SetIndex` (`minimizer & (S2_NUM_SETS - 1)`, a raw low-bit mask, no mixing) produces catastrophically uneven set occupancy: `min=0 max=165460 mean=734.02 max_over_mean=225.42` across 4,096 sets, on 3,006,550 total lookups. One set absorbed 225× the average load; at least one set was never touched. Reuse-distance histogram confirms the locality theory directly: 81% of all repeats (54% of all lookups are repeats) land at distances of 10,000–1,000,000 lookups apart (`lt1e5`=341,228, `lt1e6`=978,748) — far beyond what a 16,384-entry cache can survive, and the interaction with the occupancy bug compounds this (entries in the hottest sets get evicted almost immediately, long before surviving that many intervening lookups).

**Fix (`plan_paper/scripts/s4_0b_hash_mix_diagnostic_patch.py`, then `s4_0c_hash_mix_real_patch.py` for the real tree):** mix the minimizer through Kraken2's own `MurmurHash3` (already declared in `kv_store.h`, already `#include`d by `classify.cc`, already used by the main hash table) before masking — one line changed, zero new includes. Validated with a standalone synthetic-distribution compile-check before touching Luna (256/4096 sets used → 4096/4096, max/mean 16.06→1.84), then measured directly: **hit rate went from 0.4035% to 3.5758% (~8.9×)** at the identical 4,096-set capacity, occupancy max/mean from 225.42 to **3.95**. Ported into the real S3.3 tree, verified byte-identical `--output`/`--report` vs S0, crash-free and consistent at 16T/32T/96T. Committed+tagged on Luna as `safe/S4.0-hashmix` (`a240d60`).

**But: this does not show up as a wall-clock win either — for the same underlying reason as S3.4.** Interleaved pre/post comparison (`sample_targeted` and `standard_8gb`, T=32/T=96, 2 runs each): all four configs statistically indistinguishable (0.4–1.8% differences, within noise). One real, reproducible secondary signal: `sample_targeted`'s LLC-load-miss% dropped consistently from ~13.2% to ~9.2% (both runs, same direction) — a genuine ~30% relative reduction in that specific metric, but too small a slice of total runtime (S2's absolute contribution was always tiny, per S3.4) to move end-to-end time. The math: 95,377 additional cache hits out of ~3M total lookups is still only ~3.2% of the lookup stream, and `Get()` calls are only part of total runtime — not enough to clear the ~1–5% noise floor.

**The bigger implication — this invalidates the empirical basis for continuing S4.1 as originally scoped.** Rebuilt the pinned-eviction variant (`s2_pinned_patch.py` + the hash-mix patch on top) and re-ran the exact 2026-08-26 comparison methodology (`standard_8gb`, T=1):
