...
- **Final round:** locked consensus positions + a synthesized recommendation.

Iterate until the group is actually satisfied it has converged, not just until a round count is hit — same standard as this project's prior debate exercises (`plan_paper/s3_s4_debate_report_2026-08-27.md`, `plan_paper/two_thesis_strategy_debate_2026-08-30.md`).
...
- **S3.4** (full benchmark, 3 DBs × 6 thread counts × 3 runs): zero measurable wall-clock difference between no-cache, the original fixed-size cache, and the fully-fixed dynamic cache — anywhere. Root cause: hit rate never exceeds ~1.5% even at the largest safely-tested size (262,144 sets), so the cache's absolute contribution to total work is too small to clear ~1-5% measurement noise.
