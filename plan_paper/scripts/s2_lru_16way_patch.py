# S2-LRU-16WAY test variant (2026-09-02) - part of the associativity sweep
# (4/8/16/32/64 ways), S2_WAYS=16 (4,096 sets x 16 ways = 65,536 entries,
# ~1MB/thread at T=1). Same true recency-stamp LRU eviction as the other
...
new_sig = '''// S2-LRU-16WAY TEST VARIANT (2026-09-02) - same true recency-stamp LRU
// eviction as s2_lru_patch.py, S2_WAYS=16 instead of 4 (4,096 sets x 16
// ways = 65,536 entries, ~1MB/thread) - part of the 4/8/16/32/64
...
static const size_t S2_WAYS = 16;         // CHANGED from 4
...
static void S2PrintStats() {
  ...
  fprintf(stderr, "[S2-LRU-16WAY] size=%zu ways=%zu hits=%llu misses=%llu total=%llu hit_rate=%.4f%%\\n",
          S2_NUM_SETS, S2_WAYS,
...
new_lookup = '''            taxon = 0;
            if (! skip_lookup) {
              // S2-LRU-16WAY - same wiring as the other standalone variants,
