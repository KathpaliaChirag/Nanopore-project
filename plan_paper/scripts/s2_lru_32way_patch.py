# S2-LRU-32WAY test variant (2026-09-02) - identical to s2_lru_8way_patch.py
# with S2_WAYS raised to 32 (4,096 sets x 32 ways = 131,072 entries,
# ~2MB/thread at T=1). Part of an associativity sweep (4/8/32/64 ways)
# requested alongside the LRU eviction test, to see whether raw
# associativity keeps buying hit rate as it climbs, independent of the
# eviction-policy question. Meant to be built on top of the S4.0b
# hash-mix fix, same as the other LRU variants.
...
new_sig = '''// S2-LRU-32WAY TEST VARIANT (2026-09-02) - same true recency-stamp LRU
// eviction as s2_lru_patch.py, S2_WAYS=32 instead of 4 (4,096 sets x 32
// ways = 131,072 entries, ~2MB/thread) - part of an associativity sweep
// (4/8/32/64) to see whether hit rate keeps climbing with more ways,
// independent of eviction policy. Build on top of the S4.0b hash-mix fix.
#include <atomic>
#include <cstdio>
#include <cstdlib>

static const size_t S2_NUM_SETS = 4096;   // same set count across the whole sweep - isolates the ways variable
static const size_t S2_WAYS = 32;         // CHANGED from 4
...
static void S2PrintStats() {
  ...
  fprintf(stderr, "[S2-LRU-32WAY] size=%zu ways=%zu hits=%llu misses=%llu total=%llu hit_rate=%.4f%%\\n",
          S2_NUM_SETS, S2_WAYS,
...
new_lookup = '''            taxon = 0;
            if (! skip_lookup) {
              // S2-LRU-32WAY - same wiring as the other standalone variants,
