# S2-LRU-8WAY test variant (2026-09-02) - identical to s2_lru_patch.py
# (same standalone wiring, same true-recency-stamp LRU eviction, same
# hit/miss instrumentation) with ONE change: S2_WAYS raised from 4 to 8,
# doubling capacity at the same 4,096 sets (16,384 -> 32,768 entries,
# ~256KB/thread -> ~512KB/thread). Requested 2026-09-02 alongside the LRU
# test, as an extra "does raw associativity help independent of eviction
# policy" check. Meant to be built on top of the s4_0b hash-mix fix, same
# as s2_lru_patch.py.
...
new_sig = '''// S2-LRU-8WAY TEST VARIANT (2026-09-02) - same as s2_lru_patch.py's true
// recency-stamp LRU eviction, but S2_WAYS=8 instead of 4 (4,096 sets x 8
// ways = 32,768 entries, ~512KB/thread) - isolates "does more associativity
// help independent of eviction policy" from the LRU-vs-round-robin question.
// Meant to be built on top of the S4.0b hash-mix fix, same as s2_lru_patch.py.
#include <atomic>
#include <cstdio>
#include <cstdlib>

static const size_t S2_NUM_SETS = 4096;   // same set count as the 4-way variants - isolates the ways variable
static const size_t S2_WAYS = 8;          // CHANGED from 4
...
static void S2PrintStats() {
  ...
  fprintf(stderr, "[S2-LRU-8WAY] size=%zu ways=%zu hits=%llu misses=%llu total=%llu hit_rate=%.4f%%\\n",
          S2_NUM_SETS, S2_WAYS,
...
new_lookup = '''            taxon = 0;
            if (! skip_lookup) {
              // S2-LRU-8WAY - same wiring as the other standalone variants,
