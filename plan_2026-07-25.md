...
- The patch containing Patch 4 also bundles a Luna-specific x86 compile flag (`-march=sapphirerapids`), which won't build on Orion's ARM64 toolchain. Patch 4 (the cache) needs separating from that flag before this thesis's cross-machine sizing work can run on Orion at all.
...
- [ ] Run it on the exact same reads, same machine, same thread counts, and same performance counters already used for every Kraken2 measurement in this project (see `dorado-kraken-research/CLAUDE.md`'s "Standard Kraken2 Profiling Command" — mirror that exactly).
...
1. Apply and benchmark `kraken2_opt_v1.patch` (the original summer patch) first, if it hasn't been already. Thesis 1 builds directly on the cache inside this patch (Patch 4), so Thesis 1 cannot start without it, and both theses benefit from having a real measured baseline instead of a projected one before layering more changes on. Also separate the x86 compile flag now (see Thesis 1's risks).
