#!/bin/bash
# Software cache SIZE sweep (independent of associativity, which stays fixed
# at 4-way throughout - the width every prior sweep found optimal/tied-optimal).
# 6 sizes (2048/4096/16384/65536/262144/1048576 sets, ~4x steps, anchored on
# the default formula's own clamp bounds 4096-262144 plus one point below and
# one above) x 2 DB sizes (50MB sample_targeted, 8GB standard_8gb) = 12 runs.
# Each binary is a real rebuild of kraken2-src-fresh (current S4.0 hashmix +
# S3.3 zero-sentinel state) with S3_MIN_SETS == S3_MAX_SETS pinned to the
# target value - this forces S3ComputeNumSets() to return exactly that value
# regardless of thread count (all runs here use -p 1/-n 1, T=1 anyway).
# Verified before this sweep was launched: all 6 binaries produce byte-identical
# classification output to S0 and to each other on the 10-read workload
# (7/10 classified) - the cache is a pure lookup optimization, size doesn't
# change correctness, only performance.
cd ~/cache_simulation/snipersim
OUTBASE=/home/student/cache_simulation/results_size_sweep
mkdir -p $OUTBASE
...
for dbname in 50mb 8gb; do
  ...
  for size in 2048 4096 16384 65536 262144 1048576; do
    ...
    echo "=== $(date '+%Y-%m-%d %H:%M:%S') Running size_sweep size=${size} db=${dbname} ===" >> $OUTBASE/progress.log
    WSTART=$(date +%s)
    ...
    echo "=== $(date '+%Y-%m-%d %H:%M:%S') Done size_sweep size=${size} db=${dbname} wallclock=${WALLCLOCK}s ===" >> $OUTBASE/progress.log
  done
done
