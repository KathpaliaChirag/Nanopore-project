...
## 2026-06-16 (cont.) - Second-pass coordinator audit (7 coordinators, 17+ sub-agents)

Follow-up to the same-day multi-agent review above; covers commits `9e6996e`, `097f345` (not the earlier three).

- Coordinator fixes: canonical wall times standardized (Step 8a/8d → 4.405s, −21.8%), section renumbering (`RESULTS.md` §4.1–4.7), date corrections, three-machine Verdict table in `docs/Luna_vs_Minerva.md`.
- Deep-dive audit (V1–V6 + F1 sub-agents, all SATISFIED): behavioral-class naming standardized ("bandwidth-saturated", "Amdahl-limited"), Orion thread counts fixed (added 6, 10), AccuracyChase results table completed (96.79/98.86/99.24%), Minerva proxy61→proxy62 fix (8 occurrences).
- **`CLAUDE.md` created** - project overview, SSH targets, key paths, standard profiling commands.

---

## 2026-06-22 - AccuracyChase per-pod5 fresh basecalling + thread scaling

- Re-basecalled all 16 FBE pod5 files individually (fast+hac+sup): FAST 1,922,066 / HAC 1,906,966 / SUP 1,923,965 total reads.
- Per-pod5 Kraken2 classification - 96-run experiment (16 pod5 × 3 models × 2 DBs at 1T).
- Thread scaling 2T–96T for reads_hac × sample_targeted, reads_fast × 3 DBs, and pluspf_103gb warm scaling for hac/sup/fast (Luna).
- Files: `AccuracyDrift/AccuracyChase.md`, `COMMANDS.md`, `RESULTS.md`.

---

## 2026-06-24 - M1–M4 patch-decision characterisation

- New `AccuracyDrift/patches.md`: M1 (all DBs ≈0.70 load factor, 32-bit cells except sample_targeted's 26+6, PF_STRIDE=16 confirmed) and M2 (dTLB-load-miss 0.05–0.32% raw) → **Patch 2 (MADV_HUGEPAGE) and Patch 3 (prefetch): apply**.
- M3: cross-DB LLC-miss composition - kernel page-fault/page-walk handler is the dominant miss line (33–75% depending on DB size), not `CompactHashTable::Get()` itself (2–11%) - motivates huge pages more than originally thought.
- M4: DRAM bandwidth 5.9–11% of DDR5 peak across all DBs → confirmed latency-bound, not bandwidth-bound.
- Fixed Luna source/binary path errors in `CLAUDE.md`.

---

## 2026-06-26 - README 5-agent overhaul + M1–M7 completion + Dorado L40S GPU profiling

- `be30c93`: comprehensive 5-agent README pipeline - expanded README from ~10-11 sections to **15 numbered sections** + Colab notebook link (hardware table, Dorado 3-platform timing, LLC reversal explanation, species table, patch diagnosis table, tiered TODOs, repo structure).
- Added Key Numbers at-a-glance table, M4 DRAM bandwidth + IPC decay chart, sys-time growth + A. baumannii model-sensitivity data.
- **M1–M7 all complete except M6**: M5 k-mer reuse rate = **90.7%** (32.8M unique / 351.8M lookups) → Patch 4 (Kolin sir's LRU cache) revised **−20% → −40–50%**. M7: `objdump` shows 0 AVX-512, 0 AVX2, 1308 SSE instructions in `classify` - zero vectorisation → Patch 1 (compile flags) revised **−8% → −15–25%**. All 4 patches now a **go**; wall-time projection 4.405s → ~1.92s (not yet measured).
- Dorado profiled on Luna's 2×L40S despite the Meeting 4 decision to focus on Kraken2 only: fast=33.9s, hac=55s, sup=4m26s wall-time; nsys kernel breakdown (hac: LSTM 68%; sup: FP8+FP16 GEMM 43.6%, RMSNorm 17.1%, attention 10.4%, 0% LSTM - Transformer arch). Had to install `cuda-nsight-systems-12-9` separately - default apt nsys/nvprof broken on sm_89 (L40S). Results in new file `AccuracyDrift/dorado_profiling.md`.
- `CLAUDE_RECAP.md` created.

---

## 2026-06-27 - Dorado CPU/GPU/Orion comparison + pod5 ESKAPE tables

- Full cross-platform Dorado timing table: Luna CPU fast=9m40s (28.6× slower than GPU) / hac=43m26s (107×) / sup≈9 days estimated (SUP's FP8 tensor-core ops have no CPU fallback path - including Xeon Platinum 8468 AMX - so they fall back to FP32 and lose fused-kernel benefits). Orion (Jetson AGX Orin) GPU: fast=6m44s, hac≈1 day (corrected in `daa909f` from an earlier erroneous "24 min" figure).
- Per-pod5 ESKAPE species classification comparison table filled for all 16 pod5 files (HAC only), comparing sample_targeted (50 MB) vs pluspf_103gb DB.
- Fixed a data-ordering bug from parallel greps that had misassigned results for pods 5–14 (`f8d9241`).

---

## 2026-07-01 - Merged-pod5 full-dataset profiling

- All 16 FBE pod5 files merged into one dataset via dorado directory input (~1.87M reads total) - GPU throughput identical to the single-file baseline (already saturated regardless of dataset size).
- Kraken2 run on the merged set across 3 models × 2 DBs (sample_targeted, pluspf_103gb): classification rate jumps sharply with the bigger DB - fast 80.67%→96.53%, sup 85.46%→99.32%. LLC miss rate 12–13% on the 50 MB DB vs 62–75% on the 103 GB DB (warm re-run confirmed 75.44%).
- New file: `AccuracyDrift/merged_pod5_profiling.md` (nsys/CUDA profiling sections still placeholder `???`, not yet filled).

---

## 2026-07-02 - Presentation deck + recap command

- `presentations/june.pptx` added - 26-slide project status deck, plus supporting `presentation/flamegraph_hac_32t.svg`.
- Per-pod5 HAC raw perf/report dumps committed under `AccuracyDrift/runs/fbe_pod5_hac/{0..15}/`.
- New `.claude/commands/recapupdate.md` slash command for keeping `CLAUDE_RECAP.md` current going forward.

---

## 2026-07-03 - Docs reconciliation (multi-agent audit)

- **Goal:** `CLAUDE.md` and `CLAUDE_RECAP.md` had drifted badly out of sync with actual project state - both still claimed M1–M7 "not run" and Dorado L40S profiling "deprioritised," when git history showed both were done weeks earlier (`0eb98c3`, `a05da96`+). `docs/updates.md` itself had a 2.5-week gap (2026-06-16 → 2026-07-02, ~35 commits unlogged).
- Four parallel research agents audited: (1) M1–M7 + patch status, (2) Dorado GPU profiling status, (3) AccuracyDrift pod5/merged-pod5 completion, (4) the full commit gap in this file.
- **Corrections applied:**
  - `CLAUDE.md`: "What Is Not Done Yet" rewritten - top remaining task is now *applying and benchmarking the patch* (M1–M7 done, all 4 patches are a go); Dorado L40S moved from "deprioritised" to "done"; Repository Architecture section updated with `AccuracyDrift/patches.md`, `dorado_profiling.md`, `merged_pod5_profiling.md`, `pod5_classification_comparison.md`, `runs/fbe_pod5_hac/`; behavioral classes updated from three to four (added DRAM-saturated / pluspf_103gb).
  - `Luna/experiments/pending_measurements.md`: stale "none of M1-M7 have been run yet (2026-06-15)" header corrected.
  - `CLAUDE_RECAP.md`: fully rewritten to reflect true current state (see file for details).
  - This file: backfilled with the 8 session entries above.
- **Still genuinely open** (confirmed, not just re-labeled): the patch has never been applied/benchmarked; Orion `reads_fast` untouched; Luna `reads_fast` missing eskape_650mb and eskape_human_4gb; `AccuracyDrift/machines/Luna.md` doesn't exist; `merged_pod5_profiling.md` nsys sections are placeholder.
