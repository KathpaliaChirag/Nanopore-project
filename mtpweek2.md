...
> [!NOTE]
> **2026-08-10 — considered skipping Orion entirely after Luna's numbers came in slow.** Both Metabuli and Centrifuger turned out clearly slower than Kraken2 on Luna once real numbers were in hand, which raised the question of whether Orion was worth attempting at all this week. Two points worth keeping on record before making that call:
> - Metabuli has a prebuilt ARM64 binary already — checking it works there is a near-zero-cost step (download, extract, run a version check), not a real time investment. Skipping it doesn't save much.
> - Centrifuger is the one item this week's plan flagged as the actual unknown worth testing — nobody has ever confirmed whether it builds on ARM64 at all, and the plan explicitly says either outcome ("builds clean" or "fails, here's why") is a real, useful finding for a thesis with a cross-hardware angle. It's genuinely optional though — the week's definition of done doesn't require it, only Luna does.
>
> Speed on Luna (x86) doesn't predict ARM64 build/run compatibility — those are different questions. Recorded here so the reasoning survives regardless of which way the call goes.
