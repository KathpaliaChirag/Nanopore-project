...
> [!IMPORTANT]
> **Correction (2026-08-27):** the "free padding" claim below is wrong as originally written. A 5-agent/3-round debate exercise ([`plan_paper/s3_s4_debate_report_2026-08-27.md`](https://github.com/KathpaliaChirag/Nanopore-project/blob/main/plan_paper/s3_s4_debate_report_2026-08-27.md), Q3) verified `taxid_t` directly against Kraken2's upstream source (`src/kraken2_data.h`) and found it's `uint64_t` (8 bytes), not the 4 bytes assumed here. The *original* `S2Entry` (`tag`+`taxon`) is 16 bytes with **zero** padding — the 16-byte figure was always right, but for a different reason (8+8, not 8+4-padded). Adding `was_hit` genuinely grew the struct to **24 bytes** (a real, previously-unflagged 50% increase, also breaking the original's clean one-cache-line-per-set alignment), not "into pre-existing padding." The saturating-counter recommendation below still holds, but only relative to the *already-24-byte* pinned struct (7 bytes of real slack there), not the original 16-byte design. See the debate report for the full corrected accounting before implementing this.

**Free bits are actually available, not just theoretically — but see the correction above for the real byte accounting.** `S2Entry` (from `plan_paper/scripts/s2_pinned_patch.py`) is `{ uint64_t tag; taxid_t taxon; bool was_hit; }`. The confirmed 16-bytes/entry arithmetic (`4,194,304 × 4 × 16 = 268,435,456`) is for the *original* struct before `was_hit` was added — with `taxid_t` confirmed as `uint64_t`, that struct is exactly `tag`(8)+`taxon`(8)=16 bytes with no padding. `was_hit` pushed the pinned variant to 24 bytes, leaving 7 bytes of genuine slack there. **A saturating counter fits in that already-paid-for space.** Concretely: replace `bool was_hit` with `uint8_t freq` (0–3, 2 bits used, same 1 byte), so the pinned entry stays at its current 24 bytes — S3's sizing math must use 24 bytes/entry for anything built on this struct, not 16.
