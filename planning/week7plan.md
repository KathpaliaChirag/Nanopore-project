...
**Real S2 lookup is a sequential AoS scan, not a parallel comparator — CK's second hypothesis on the associativity story.** `S2Lookup`/`S2Insert` (e.g. `plan_paper/scripts/s2_lru_4way_noatomics_patch.py:43-66`) do a plain `for (way=0; way<S2_WAYS; way++)` over an array-of-structs, comparing tags one at a time on every call including misses — real hardware set-associative caches instead fire all N ways' comparators in parallel, so lookup latency doesn't scale with ways the way this software loop's wall-clock does. This means the 2026-09-02 sweep's "more ways = better hit rate, worse wall-clock" result is at least partly confounded by the scan itself getting longer, on top of the allocation-cost effect the 2026-09-02 debate already locked as dominant (see the note above and `plan_paper/research_brief_associativity_case_study_2026-09-02.md`, where this is item (ii) in the "Proposed sequence" / debate outcome — SIMD/SoA fingerprint matching, informed by SwissTable/F14, already explicitly evaluated and dropped for this cycle, "revisit post-Sept-13").

A first cut now exists: `plan_paper/scripts/s2_lru_4way_simd_patch.py` — SoA 1-byte-fingerprint array per set (`S2_WAYS` fingerprints packed contiguous, tag/taxon/last_used kept in a separate parallel array), lookup does an unrolled branchless compare across all 4 fingerprints into a match mask before touching the full 64-bit tag, no atomics. Written, **not built, not benchmarked** — same status as every other unbuilt S2 variant in this repo until it's actually run on Luna.

**Priority: below item 4 (Sniper/TEJAS simulator case study), which stays the active track per CK's explicit call today.** Rationale for staying deferred: item 4 answers the same underlying question (is associativity a real win once implementation confounds are gone) more cleanly, since a simulator removes the confound by construction rather than requiring a hand-rolled SIMD rewrite to be verified correct first. If item 4's simulator result comes back ambiguous or shows a real hardware-level effect worth cross-checking on real silicon, this becomes the natural follow-up — build, benchmark against the existing noatomics AoS variants, and fold into the case study's three-way comparison (S0 vs. AoS-simulated vs. SIMD/SoA) already sketched in the research brief's step 5.
