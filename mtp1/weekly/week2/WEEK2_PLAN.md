...
### Worked example: how a metamer survives an indel that breaks DNA-only matching

> [!NOTE]
> This is a hand-built teaching example — a short toy gene, not real capture data or a benchmark run. The numbers exist to make the mechanism concrete, not to report a measured result. Real Metabuli aggregates many metamers per read through an actual scoring/E-value formula, not the simplified counting shown here.

**The setup.** Two reference species in the database whose genes differ at the DNA-letter level but — because several different 3-letter DNA codons can translate to the same amino acid — encode the *identical* protein. This is realistic: closely related bacterial strains do this routinely (synonymous, "wobble-position" variation).
...
If you only ever compare proteins, Taxon 100 and Taxon 200 are indistinguishable here. That's the point of the example — it's built to force a tie at the amino-acid level.

**The read.** The sample really is Taxon 100, but the Nanopore basecaller drops one base — the `G` that should start the 5th codon (`GAT`):
...
Everything after the missing base is now one letter out of position relative to the true gene — the letters themselves are untouched, just shifted.

**Step 1 — reading in the original frame produces garbage.** A classifier locked to "codons start at letter 1" keeps grouping in 3s straight through the gap:
...
Codons 1–4 are fine (they're before the missing letter); codon 5 onward grabs letters that used to belong to different codons. `I H L...` matches nothing in the database — this is also exactly where a fixed-frame DNA k-mer match dies.

**Step 2 — a different starting offset re-syncs with the truth.** The letters after the gap weren't destroyed, only their grouping was knocked out of phase. Shifting the codon boundaries by 2 letters recovers the real protein:

| Letters | Amino acid | True Taxon 100 amino acid here |
|---|---|---|
| CCA | P | P ✓ |
| TTT | F | F ✓ |
| GGA | G | G ✓ |
...
`P F G N Q E W H` — every codon matches. Nothing about the DNA changed between Step 1 and Step 2, only where the grouping starts. This is the payoff of trying multiple reading frames instead of one fixed frame: one of them re-syncs with the truth. This 8-amino-acid chunk, plus its underlying raw DNA, is a **metamer** — Metabuli's basic comparison unit.

**Step 3 — the protein match ties; the DNA letters break the tie.** `P F G N Q E W H` fits both Taxon 100 and Taxon 200 equally (by construction). But the raw 24 DNA letters underneath that chunk aren't identical between the two:
...
The read's DNA lines up perfectly with Taxon 100 and only partially with Taxon 200 — even with identical proteins. This letter-by-letter agreement is the **DNA-identity score**.

**Step 4 — combining both signals.** Protein match: tied between the two taxa. DNA-identity: Taxon 100 wins clearly (100% vs ~71%). Combined, the evidence correctly points to Taxon 100 — the true source. Protein-only matching would have left this ambiguous; DNA-only matching, locked to one frame, would have lost this whole region to the sequencing error in the first place. A real read produces many metamers like this one, and Metabuli sums the evidence across all of them before making its final per-read call.
